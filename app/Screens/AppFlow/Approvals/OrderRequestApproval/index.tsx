...
import { filterDataByName } from "../../../../Utils/globalFunction";
...
const OrderRequestApproval = (props: any) =>{
  ...
  const onChangeBtn = (item: any) => {
    onChangeBtnPress(screen, item, dispatch, getDetail, props.route?.params);
  };
  ...
  const filterLogic =()=>{
    if(screen ==1){
      const initialReadyData = readyApprovedList?.consumeReqHdrDtl?.slice(0, itemsPerPage);
      setFilterDataReady(initialReadyData);
     const initialPartialData = partialApprovedList?.consumeReqHdrDtl?.slice(0, itemsPerPage);
     setFilterDataPartial(initialPartialData);
     const initialOutofStockData = outofStockList?.consumeReqHdrDtl?.slice(0, itemsPerPage);
     setFilterDataOutofStock(initialOutofStockData);
    }else if (screen == 2){
      const initialReadyData = readyApprovedList?.orderApprovalHdr?.slice(0, itemsPerPage);
      setFilterDataReady(initialReadyData);
      const initialPartialData = partialApprovedList?.orderApprovalHdr?.slice(0, itemsPerPage);
      setFilterDataPartial(initialPartialData);
    }else{
      const initialReadyData = readyApprovedList?.orderApprovalHdr?.slice(0, itemsPerPage);
      setFilterDataReady(initialReadyData);
      const initialPartialData = partialApprovedList?.orderApprovalHdr?.slice(0, itemsPerPage);
      setFilterDataPartial(initialPartialData);
      const initialOutofStockData = outofStockList?.orderApprovalHdr?.slice(0, itemsPerPage);
     setFilterDataOutofStock(initialOutofStockData);
    }
  };
  useEffect(() => {
  ...
  },[selectedIndex,searchKey,readyApprovedList,partialApprovedList,outofStockList]);

  const onScrollLoadRestData = () => {
    setApploader(true);
    setTimeout(()=>{ setApploader(false);}, 200);
   if (screen == 1) {
      if (selectedIndex == 2 && filterDataReady.length < readyApprovedList?.count) {
        const nextPageData = readyApprovedList?.consumeReqHdrDtl?.slice(currentPageReady * itemsPerPage, (currentPageReady + 1) * itemsPerPage);
        setFilterDataReady([...filterDataReady, ...nextPageData]);
        setCurrentPageReady(currentPageReady + 1);
      } else if (selectedIndex == 3 && filterDataPartial.length < partialApprovedList?.count) {
        const nextPageData = partialApprovedList?.consumeReqHdrDtl?.slice(currentPagePartial * itemsPerPage, (currentPagePartial + 1) * itemsPerPage);
        setFilterDataPartial([...filterDataPartial, ...nextPageData]);
        setCurrentPagePartial(currentPagePartial + 1);
      } else if (selectedIndex == 4 && filterDataPartial.length < outofStockList?.count) {
        const nextPageData = outofStockList?.consumeReqHdrDtl?.slice(currentPageOutofStock * itemsPerPage, (currentPageOutofStock + 1) * itemsPerPage);
        setFilterDataOutofStock([...filterDataOutofStock, ...nextPageData]);
        setCurrentPageOutofstock(currentPageOutofStock + 1);
      }
    } else if (screen == 2) {
      if (selectedIndex == 2 && filterDataReady.length < readyApprovedList?.count) {
        const nextPageData = readyApprovedList?.orderApprovalHdr?.slice(currentPageReady * itemsPerPage, (currentPageReady + 1) * itemsPerPage);
        setFilterDataReady([...filterDataReady, ...nextPageData]);
        setCurrentPageReady(currentPageReady + 1);
      } else if (selectedIndex == 3 && filterDataPartial.length < partialApprovedList?.count) {
        const nextPageData = partialApprovedList?.orderApprovalHdr?.slice(currentPagePartial * itemsPerPage, (currentPagePartial + 1) * itemsPerPage);
        setFilterDataPartial([...filterDataPartial, ...nextPageData]);
        setCurrentPagePartial(currentPagePartial + 1);
      }
    }
      ...
      if (selectedIndex == 2 && filterDataReady.length < readyApprovedList?.count) {
        ...
        setFilterDataPartial([...filterDataPartial, ...nextPageData]);
        setCurrentPagePartial(currentPagePartial + 1);
      } else if (selectedIndex == 4 && filterDataOutofStock.length < outofStockList?.count) {
        const nextPageData = outofStockList?.orderApprovalHdr?.slice(currentPageOutofStock * itemsPerPage, (currentPageOutofStock + 1) * itemsPerPage);
        setFilterDataOutofStock([...filterDataOutofStock, ...nextPageData]);
        setCurrentPageOutofstock(currentPageOutofStock + 1);
      }
    };
  ...
  const readyData = () => {
    if (screen == 1) {
        //const data = searchKey&&selectedIndex==2?readyApprovedList?.consumeReqHdrDtl:filterDataReady?.filter(
      const data = filterDataReady?.filter(
        (item: any) => item?.consumeApprovalStatus == "Ready To Be Approved"
      );
      return data;
    } else if (screen == 2) {
      //const data = searchKey&&selectedIndex==2?readyApprovedList?.orderApprovalStatus:filterDataReady?.filter(
        const data = filterDataReady?.filter(
  ...
  const partiallyData = () => {
    if (screen == 1) {
        //const data = searchKey&&selectedIndex==3?partialApprovedList?.consumeReqHdrDtl:filterDataPartial?.filter(
          const data = filterDataPartial?.filter(
        (item: any) => item?.consumeApprovalStatus == "Partially Available"
      );
      return data;
    } else if (screen == 2) {
        // const data = searchKey&&selectedIndex==3?partialApprovedList?.orderApprovalHdr:filterDataPartial?.filter(
          const data = filterDataPartial?.filter(
  ...
  const outOfStockData = () => {
    if (screen == 1) {
        // const data = searchKey&&selectedIndex==3?outofStockList?.consumeReqHdrDtl:filterDataOutofStock?.filter(
          const data = filterDataOutofStock?.filter(
        (item: any) => item?.consumeApprovalStatus == "out of stock"
      );
      return data;
    } else if (screen == 3) {
        // const data = searchKey&&selectedIndex==4?outofStockList?.orderApprovalHdr:filterDataOutofStock?.filter(
          const data = filterDataOutofStock?.filter(
